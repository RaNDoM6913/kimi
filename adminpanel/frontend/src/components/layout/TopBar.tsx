...
interface TopBarProps {
  pageTitle: string;
  /** Renders between page title and search (e.g. Moderation "6 pending" badge) */
  leftSlot?: React.ReactNode;
}

export function TopBar({ pageTitle, leftSlot }: TopBarProps) {
  ...
  const getAlertIcon = (type: Alert['type']) => {
    switch (type) {
      ...
      case 'warning':
        return <AlertTriangle className="w-4 h-4 text-[#FFD166]" />;
      case 'success':
        return <Check className="w-4 h-4 text-[#2DD4A8]" />;
      case 'info':
        return <Info className="w-4 h-4 text-[#4CC9F0]" />;
    }
  };

  return (
    <header className="h-16 bg-[#0E1320] border-b border-[rgba(123,97,255,0.12)] sticky top-0 z-40">
      <div className="h-full w-full flex items-center px-4 sm:px-6">
        <div className="flex items-center w-full gap-4">
          ...
          <div className="flex items-center gap-2 md:gap-4 min-w-0 flex-1 justify-end">
            {leftSlot ? <div className="shrink-0 ml-2">{leftSlot}</div> : null}

            {/* Search (адаптив) */}
            <div className="hidden lg:block min-w-0 w-full max-w-[280px] xl:max-w-[520px]">
              <div
                className={cn(
                  'relative transition-all duration-300 min-w-0',
                  searchFocused ? 'max-w-[520px]' : 'max-w-[420px]'
                )}
              >
                <Search
                  className={cn(
                ...
                <input
                  ...
                  className={cn(
                    'w-full min-w-0 pl-10 pr-10 py-2 rounded-lg text-sm transition-all duration-200',
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {!alert.isRead && <span className="w-2 h-2 bg-[#7B61FF] rounded-full" />}
                              </div>
